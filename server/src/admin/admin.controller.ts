...
export class AdminController {
  ...
  @Get("trips")
  @ApiOperation({
    summary: "List trips with optional status filter and pagination",
  })
  @ApiQuery({
    name: "status",
    required: false,
  ...
  approveTrip(@Param("id") id: string, @Req() req) {
    return this.adminService.approveTrip(
      id,
      req.user.id,
  ...
  rejectTrip(@Param("id") id: string, @Body() dto: RejectTripDto, @Req() req) {
    return this.adminService.rejectTrip(
      id,
      dto.reason,
      req.user.id,
      buildAuditContext(req),
  ...
  @ApiParam({
    name: "id",
    description: "ID of the user to set the temporary password for",
  })
  @ApiResponse({
    status: 200,
  ...
  setTemporaryPassword(
    ...
    return this.adminService.setTemporaryPassword(
      userId,
